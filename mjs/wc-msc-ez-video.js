...
const mediaTrackAction = (ezVideo, action = 'previous', ezVideos = getAllEzVideo()) => {
  const idx =
    (ezVideos.indexOf(ezVideo) + (action === 'previous' ? -1 : 1) + ezVideos.length) %
    ezVideos.length;
  const actEzVideo = ezVideos[idx];
  const tagName = _wcl.classToTagName('MscEzVideo');

  // pause all
  Array.from(document.querySelectorAll(`${tagName},video,audio`)).forEach(mediaObj =>
    mediaObj.pause(),
  );
...
export class MscEzVideo extends HTMLElement {
  ...
  _onError() {
    ...
    // custom event
    this.dispatchEvent(
      new CustomEvent(custumEvents.error, {
        bubbles: true,
        composed: true,
        detail: {
          error,
